//Shameless copy of https://github.com/pacman82/arrow-odbc-py/blob/main/src/error.rs

use std::{ffi::CString, fmt::Display, os::raw::c_char, ptr::NonNull};

/// Handle to an error emmitted by arrow odbc
pub struct RustPythonError {
    message: CString,
}

impl RustPythonError {
    pub fn new(source: impl Display) -> RustPythonError {
        ...
        RustPythonError { message }
    }

    /// Moves the instance to the heap and return a pointer to it.
    pub fn into_raw(self) -> *mut RustPythonError {
        Box::into_raw(Box::new(self))
    }
}

impl<T> From<T> for RustPythonError
where
    T: Display,
{
    fn from(source: T) -> RustPythonError {
        RustPythonError::new(source)
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn rust_python_error_free(error: NonNull<RustPythonError>) {
...
/// Error must be a valid non null pointer to an Error.
#[no_mangle]
pub unsafe extern "C" fn rust_python_error_message(error: *const RustPythonError) -> *const c_char {
    let error = &*error;
    ...
    ($call:expr) => {
        match $call {
            ...
            Err(error) => {
                ...
                let error = Into::<RustPythonError>::into(error);
